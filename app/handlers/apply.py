...
async def handleApply(update: types.CallbackQuery | types.Message, state: FSMContext):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                text=_("Посетить сайт СНМК"), url=_("Сайт СНМК, ссылка")
            )
        ],
    ...
    if isinstance(update, types.CallbackQuery):
        return await update.message.edit_text(text=text, reply_markup=reply_markup)
    elif isinstance(update, types.Message):
        return await update.answer(text=text, reply_markup=reply_markup)
